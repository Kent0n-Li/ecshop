...
class Order extends NotORM
{
    ...
    function order_due_field($alias = '')
    {
        ...
        //                " - {$alias}bonus ";
        return $this->order_amount_field($alias) .
            " - {$alias}money_paid  - {$alias}integral_money" .
            " - {$alias}bonus ";
    }
    ...
    function gen_order($user_id, $address_id, $shipping_id, $bonus_id = '', $point = 0, $tax_data = [], $pack_name, $pack_fee, $ral, $integralGoodsid, $o_time, $super, $superGoodsid, $delivery_id, $delivery_name, $result_spike, $goods_id, $p_type, $pintuan, $pintuanGoodsId, $pintuanNumber, $productId, $pintuanOrderId, $msg)
    {
        ...
        if ($ral != 'true' && $super != 'true' && $result_spike != 'true' && $pintuan != 'true') {
            ...
            // 更改秒杀订单金额
            $update_spike_price = "update ecs_order_info set goods_amount = '$goods_number' where order_id = '" . $order_id . "'";
            ...
            // 更改拼团订单金额
            $update_pt_price = "update ecs_order_info set goods_amount = '" . $pt_data['pt_price'] . "' where order_id = '" . $order_id . "'";
        ...
        if ($shipping_id > 0 && $total['real_goods_count'] > 0) {
            ...
            if (!empty($shipping_info)) {
                ...
                if (isset($productId) && !empty($productId)) {
                    $total['shipping_fee'] = ($shipping_count == 0 and $weight_price['free_shipping'] == 1) ? 0 : $this->shipping_model->shipping_fee($shipping_id, $weight_price['weight'], $total['goods_price'], $weight_price['number']);
                } else {
                    $total['shipping_fee'] = ($shipping_count == 0 and $weight_price['free_shipping'] == 1) ? 0 : $this->shipping_model->shipping_fee($shipping_id, $weight_price['weight'], $total['goods_price'], $weight_price['number']);
                }
    ...
    function cart_weight_price($user_id, $goods_count)
    {
        ...
        $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
            ...
            "AND rec_type = '0' AND g.is_shipping = 0 AND c.extension_code != 'package_buy'";
    ...
    function get_total_bonus($user_id)
    {
        ...
        $amount = floatval($amount_data['total']);
    ...
    function doPaymentBalance($order_id, $user_id, $pay_id, $platform)
    {
        ...
        $this->accountlog->log_account_change($user_id, $order_data['total_fee'] * (-1), 0, 0, 0, sprintf('订单支付', $order_id), '99', $order_id);
    ...
    public function getOrderListAction($user_id, $status, $page)
    {
        ...
        if ($status == 0) {
            ...
            foreach ($data as $k => $v) {
                ...
                if ($data[$k]['order_status'] == '4' && $data[$k]['shipping_status'] == '1' && $data[$k]['pay_status'] == '2') {
                    $data[$k]['type'] = 'cancelled';
                    $data[$k]['type_status'] = '退换货';
                }
        ...
        if ($status == 2) {
            $sql = "SELECT order_sn AS order_id,order_amount AS total_amount,user_id, pay_status,shipping_status ,order_id AS id,money_paid 
                    FROM ecs_order_info WHERE user_id = {$user_id} AND (shipping_status = 0 OR shipping_status = 3) AND pay_status = 2
            ...
            $sql = "SELECT COUNT(order_sn) AS num FROM ecs_order_info WHERE user_id = {$user_id} AND (shipping_status = 0 OR shipping_status = 3) AND pay_status = 2 ";
            $page_total = $this->getORM()->queryRows($sql);
        ...
        if ($status == 3) {
            $sql = "SELECT order_sn AS order_id,order_amount AS total_amount,user_id , pay_status,shipping_status ,order_id AS id,money_paid 
                    FROM ecs_order_info WHERE user_id = {$user_id} AND shipping_status = 1 AND pay_status = 2
                    order by add_time desc limit {$page},{$page_size}
                    ";
            $data = $this->getORM()->queryRows($sql);
            foreach ($data as $k => $v) {
    ...
    public function pintuan_fee($goods_id, $number)
    {
        $return = [];
        // 计算拼团的商品金额
        $sql = "select pt_price as goods_price from ecs_goods where goods_id = '$goods_id'";
        $pt_data = $this->getORM()->queryRow($sql);
