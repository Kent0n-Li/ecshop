...
class Category extends NotORM
{
    ...
    public function getCategoryList($cat_id)
    {
        ...
        foreach ($data as $key => $item) {
            ...
            if ($item['cate_img'] != '') {
                $return['cat_list'][$item['cat_id']]['pic'] = goods_img_url($item['cate_img']);
            } else {
                $return['cat_list'][$item['cat_id']]['pic'] = '';
            }
        }
        if ($cat_id == '') {
        ...
        foreach ($tmp as $i => $j) {
            ...
            $return['cat_item'][$j['cat_id']] = array(
                'cat_name' => $j['cat_name'],
                'cat_id' => $j['cat_id'],
                'littleTitle' => $j['cat_name'],
            );
            if ($j['cate_img'] != '') {
                $return['cat_item'][$j['cat_id']]['pic'] = goods_img_url($j['cate_img']);
            } else {
                $return['cat_item'][$j['cat_id']]['pic'] = default_category_img();
            }
        }
    ...
    public function getCategoryListGoods($cat_id, $page, $order, $type, $user_id)
    {
        ...
        $sql = "SELECT goods_id AS id,goods_thumb AS list_pic_url,goods_name AS name,shop_price AS retail_price,market_price,virtual_sales FROM ecs_goods WHERE cat_id={$cat_id} and is_on_sale = 1 and is_delete = 0 and goods_number>0 order by {$sort} {$order} LIMIT {$offset},{$page_size}";
        ...
        foreach ($goods_list as $k => $v) {
            ...
            foreach ($res as $key => $value){
                $mysql = "SELECT goods_id AS id,goods_thumb AS list_pic_url,goods_name AS name,shop_price AS retail_price,market_price,virtual_sales FROM ecs_goods WHERE goods_id = '".$value['goods_id']."'";
                ...
                $result['retail_price'] = sprintf("%.2f", $this->goods_model->get_final_price($v['id'], 1, $user_id, true, array()));
                $goods_list[] = $result;
            }
            $goods_list[$k]['list_pic_url'] = goods_img_url($goods_list[$k]['list_pic_url']);
            $goods_list[$k]["show_marketprice"] = $show_marketprice;

            $goods_list[$k]['retail_price'] = sprintf("%.2f", $this->goods_model->get_final_price($v['id'], 1, $user_id, true, array()));
        }
    ...
    public function getIndexTop()
    {
        ...
        $data = $this->getORM()->where("is_top", 1)->select("cat_id as id,cat_name,cate_img")->limit(10)->fetchAll();
        foreach ($data as $k => $v) {
