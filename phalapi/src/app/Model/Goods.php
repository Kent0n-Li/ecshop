...
class Goods extends NotORM
{
    ...
    public function spikeGoodsSum($goods_id)
    {
        ...
        //        var_dump($day,$hour,$minute,$second);
        //        die;
...
//        die;
    ...
    public function searchGoods($words, $order = 'asc', $page, $num, $user_id)
    {
        ...
        $sql = "SELECT goods_id AS id,goods_name AS name,shop_price AS retail_price,goods_thumb AS list_pic_url FROM ecs_goods WHERE is_on_sale = 1 AND is_delete = 0 and goods_name LIKE '%{$words}%' ORDER BY shop_price {$order} LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryRows($sql);
    ...
    public function getNewGoods($user_id)
    {
        ...
        //        $data = $this->getORM()->order("add_time DESC")->limit(5)->fetchAll();
        foreach ($data as $k => $v) {
    ...
    {
        // 查询当前的价格
        $goods_price_sql = "select shop_price from ecs_goods where goods_id = '" . $goods_id . "'";
        ...
        if (isset($user_id) && !empty($user_id)) {
            ...
            // 等级金额设置可为零
            if (!empty($price)) {
                return $price['user_price'];
            }

            // 如果当前用户的等级为 非特殊等级，则按照原价返回
            if ($user_rank == '0') {
                return $goods_data['shop_price'];
            }
    ...
    public function getSpikeGoodsList($page = 1)
    {
        $page_size = 12;
        $offset = ($page - 1) * $page_size;
        $sql = "SELECT goods_id,goods_name,shop_price,goods_thumb as list_pic_url,goods_number FROM ecs_goods WHERE is_new = '1' LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryAll($sql);
    ...
    public function gethotGoodsList($page = 1)
    {
        $page_size = 12;
        $offset = ($page - 1) * $page_size;
        $sql = "SELECT goods_id,goods_name,shop_price,goods_thumb as list_pic_url,goods_number FROM ecs_goods WHERE is_hot = '1' LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryAll($sql);
